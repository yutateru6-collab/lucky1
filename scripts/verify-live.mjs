/** Exact-byte verification of the deployed app, not a deployment trigger. */
import { readFile, writeFile } from 'node:fs/promises';
import { createHash } from 'node:crypto';
const origin = 'https://lucky1.itisnowornever271.workers.dev/';
const paths = ['index.html', 'app.mjs', 'styles.css', 'sw.js', 'manifest.webmanifest'];
const hash = value => createHash('sha256').update(value).digest('hex');
const expected = Object.fromEntries(await Promise.all(paths.map(async path => [path, hash(await readFile(new URL(`../public/${path}`, import.meta.url)))])));
let report;
