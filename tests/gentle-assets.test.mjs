...
test('gentle shell has unique IDs and no third-party markup', async () => {
  const html = await read('index.html');
  assert.doesNotMatch(html, /(?:src|href)=["']https?:\/\//i);
  assert.doesNotMatch(html, /\son(?:click|load|error|submit)\s*=/i);
  const ids = [...html.matchAll(/\bid="([^"]+)"/g)].map(m => m[1]);
  assert.equal(ids.length, new Set(ids).size);
  assert.ok(html.includes('data-version="4.0.0"'));
});
test('gentle uses text-only personal content and no note transmission', async () => {
