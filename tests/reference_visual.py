...
try:
    ...
    with sync_playwright() as p:
        ...
        check('version is 4.0.10', page.locator('meta[name="lucky-version"]').get_attribute('content')=='4.0.10')
        ...
        for method,selector,delay in animation_checks:
            ...
            if method=='coin': page.locator('[data-quick-pick="heads"]').click()
            if method=='cards': page.locator('[data-quick-pick="right"]').click()
        ...
        ctx.set_offline(False); top(page,'home')
        page.emulate_media(color_scheme='dark')
        page.reload(); page.wait_for_selector('#quick-start-home')
        check('Lucky stays light even when the operating system prefers dark', page.locator('html').get_attribute('data-theme')=='light')
        body_bg=page.locator('body').evaluate("el=>getComputedStyle(el).backgroundColor")
        check('page background remains bright', body_bg in ('rgb(255, 250, 252)','rgb(255, 255, 255)'))
        page.screenshot(path=str(OUT/'reference-light-under-dark-os-390.png'),full_page=True)
        page.locator('#quick-start-home').click()
        backdrop_ok=page.locator('#quick-dialog').evaluate("el=>getComputedStyle(el).backgroundColor")
        check('quick modal remains bright', backdrop_ok=='rgb(255, 250, 253)')
        page.screenshot(path=str(OUT/'reference-quick-bright-390.png'),full_page=True)
        page.locator('#quick-close').click()
        check('settings no longer offers a dark theme', page.locator('#theme-dark').count()==0 and page.locator('#theme-system').count()==0)
